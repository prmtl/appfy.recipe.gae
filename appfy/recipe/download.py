...
class Recipe(object):
    ...
    def __init__(self, buildout, name, options):
        ...
        self.option_ignore_existing = get_bool_option(
            ...
            if self.option_download_only:
                ...
                for filename in os.listdir(base):
                    ...
                    if os.path.exists(dest):
                        if self.option_ignore_existing:
                            self.logger.info('Ignoring existing target: %s' % dest)
                        else:
                            self.logger.error('Target %s already exists. Either remove it or set '
    ...
    def calculate_base(self, extract_dir):
        ...
        if self.option_strip_top_level_dir:
            if len(top_level_contents) != 1:
                self.logger.error('Unable to strip top level directory because there are more '
